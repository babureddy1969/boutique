package com.example.boutique.database;
...
import com.example.boutique.model.CustomerModel;
import com.example.boutique.model.ItemModel;
import com.example.boutique.model.PaymentModel;
import com.example.boutique.model.ReportModel;
...
//        contentValues.put(EMP_ID, employee.getId());
        ...
        contentValues.put(UPDATED_DATE, t.getUpdatedDate());
        if (t.getId()>0) {
            String[] args = new String[]{t.getId()+""};
    ...
    public List<TransactionModel> getTransactionList(String sortBy) {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                t.setUpdatedDate(cursor.getString(cursor.getColumnIndex(UPDATED_DATE)));
                list.add(t);
            } while (cursor.moveToNext());
        }
        cursor.close();
